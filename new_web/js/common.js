...
$(function () {
	...
	var news_num = $('.news-ul-div li').size();
	$('.news-ul-div li').eq(news_num - 1).fadeOut(0);
	var show_news = 3;
	var exceed = news_num - show_news;

	$('.news-blk-div-a').click(function (e) {
		e.preventDefault();
	...
	$(window).load(function () {
		...
		$('.banner-title-blk-outer').css({
			'height': $('.banner-img-bg').height() + 'px'
		});
	}).resize(function () {
		$('.banner-title-blk-outer').css({
			'height': $('.banner-img-bg').height() + 'px'
	...
	$('.menu-course-li').mouseenter(function () {

		$('.a1-plus-menu-btn').fadeIn(200).css("display", "block");
	})

	$('.iframe-nav .col-menu li a').click(function (e) {
		//e.preventDefault();
	...
	$('.menu-course-li').mouseleave(function () {
		$(".a1-plus-menu-btn").fadeOut(0);
	...
	$('.mouse-btn').click(function (e) {
		e.preventDefault();
		$('body,html').stop().animate({
			scrollTop: $('.section.news').offset().top - offsetTop
		}, {
			duration: 1000,
			easing: "easeInOutQuart",
			complete: function () {
			...
			if (!$('.animated-text-blk').eq(idx).hasClass('animated')) {
				...
				if ($(window).width() >= 700) {
					if (!$('.main-banner .line').hasClass('animated')) {
						$('.main-banner .line').css({
							'width': '0%',
							'opacity': '1'
						});
						$('.main-banner .line').addClass('animated');
						$('.main-banner .line').delay(800).animate({
							'width': '100%'
						}, {
							duration: 800,
							easing: "easeInOutQuart"
					...
					$('.banner-title-blk .middle').animate({
						'opacity': '1'
					}, {
						duration: 400,
						easing: "easeInOutQuart"
				...
				$('.animated-text-blk').eq(idx).find('.animated-bar').stop().delay(200).animate({
					'width': '100%'
				}, {
					duration: 400,
					easing: "easeInOutQuart",
					complete: function () {
						$('.animated-text-blk').eq(idx).find('.animated-bar').css({
							'left': 'auto',
							'right': '0px'
						});
						$('.animated-text-blk').eq(idx).find('.lang').animate({
							'opacity': '1'
						}, {
							duration: 1000,
							easing: "easeInOutQuart"
						});
						$('.animated-text-blk').eq(idx).find('.animated-bar').stop().animate({
							'width': '0%'
						}, {
							duration: 400,
							easing: "easeInOutQuart",
	...
	$('.employment-btn').click(function (e) {
		...
		$('.employment-detail-content-blk').css({
			'display': 'none'
		});
		$('.employment-detail-content-blk-' + Number(idx + 1)).css({
			'opacity': '0',
			'display': 'inline-block'
		});
		$('.employment-detail-content-blk-' + Number(idx + 1)).animate({
			'opacity': '1'
		}, 500);
	});
	...
	$('.contact-list-container').fadeOut(0);
	$('.contact-list-container').eq(0).fadeIn(0);

	$(".enquiry-btn-a").click(function (e) {
		e.preventDefault();
		$(".enquiry-btn-a").removeClass('active');
		$(this).addClass('active');

		var idx = $(this).parent('li').index();

		$('.contact-list-container').fadeOut(0);
		$('.contact-list-container').eq(idx - 1).fadeIn(0);
