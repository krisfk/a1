        ...
        <table width="896" border="0" cellspacing="0" cellpadding="0">
          ...
          <tr>
            <td class="it">
				<table border="0" cellspacing="0" cellpadding="0" width="100%" class="form-table" >
				  ...
				  <tr>
					...
					<?php
					    ...
					    if($event['ask_retake_part'])
                {
                  
					?>
					<tr>
					<td align="left" valign="middle"><span class="style1">*</span>重考部份</td>
					<td><label for="retake_part"></label>
					  
						
						
					
						
						  <select name="retake_part" id="retake_part" style="width:200px; ">
						  <?php 
                        for($i=0;$i<sizeof($this->retake_part);$i++)
                        { 
                            $val = ($i==0) ? '0' : $this->retake_part[$i];
                            echo '<option value="'.$i.'"';
                            
                            if($i==0 && !$info['retake_part'])
                            {
                                echo 'selected';
                            }
                            else if ($info['retake_part']){
                                echo $val==$this->retake_part[$info['retake_part']] ? 'selected' : '';
                            }
                            
                            echo'>'.$this->retake_part[$i].'</option>';
                        }
                        ?>
                      ...
                      <?php if(isset($error['retake_part'])){?><span style="color:red"><?php echo $error['retake_part']?></span><?php }?>
               ...
               <tr class="coupon-code-input-tr" <?php   if(!$event['coupon_code_input']){ echo 'style="display:none;"';} ?>>
					<td align="left" valign="middle">優惠代碼</td>
					
					<td>
		<input style="width: 50px;" name="coupon_code" type="text" class="textbox" id="coupon_code_input" placeholder="" value="<?php echo $info['coupon_code'];?>" />
                      <?php if(isset($error['coupon_code'])){?><span style="color:red"><?php echo $error['coupon_code']?></span><?php }?>

						
						<!--				      <?php if($info['coupon_discount']){?><span style="color:blue"><?php echo '使用優惠代碼';?></span><?php }?>
-->	
						
						<a href="#" class="coupon-btn"> 使用代碼 </a>
						<span style="color:blue" class="use-coupon-code-txt">成功使用優惠代碼 <span class="use-coupon-code-txt-discount"></span></span>
<!--<input name="coupon_discount" class="coupon_discount" type="hidden" value="<?php echo $info['coupon_discount'] ? $info['coupon_discount']:0;?>">
--><input name="use_coupon_id" class="use_coupon_id" type="hidden" value="<?php echo $info['coupon_id'] ? $info['coupon_id']:0;?>">
          ...
          <tr>
            <td>
				<table border="0" cellspacing="0" cellpadding="0">
				  <tr>
					<td width="11" height="31" style="background:url(img/itbg_left.png) top center no-repeat;"></td>
					<td width="874" align="left" valign="middle" class="ibgt" style="background:url(img/itbg.png) top left repeat-x;">學生資料 <span class="style5">*</span>必須提供正確資料、部份資料將用作申請運輸署文件</td>
					<td width="11" style="background:url(img/itbg_right.png) top center no-repeat;"></td>
				  </tr>
				</table>
			</td>
          ...
          <tr>
			...
			<table border="0" cellspacing="0" cellpadding="0" width="100%" class="form-table form-table-2">
                ...
                <td   width="121" align="left" valign="middle" style="width: 220px;"> 
				  <span class="style1">*</span>學生姓名<br/> (身份證上的英文全名)
				</td>
                <td width="385">
					<table border="0" cellspacing="0" cellpadding="0">
					  <tr>
...
var coupon_discount = <? echo $info['coupon_discount'] ? $info['coupon_discount'] : 0; ?>;
