...
class App extends MY_Controller {
	...
	public function check_coupon_input()
	{
		...
		if($code){
				$coupon = $this->Meg_model->input_coupon_discount($code,$course_id);
			
				//if($coupon)
		//		{
					echo $coupon;
	//				$data['info']['coupon_discount'] = $coupon['discount'];
...
//	}
	...
	public function test($event_code= NULL){
		...
		/*$input_coupon_code='ABCD';
		$course_id="552";
		
		$sql_1 ='SELECT meg_coupon.id , meg_coupon.coupon_name ,meg_coupon.coupon_code , meg_coupon.discount , meg_coupon_course.course_id FROM meg_coupon LEFT JOIN meg_coupon_course ON meg_coupon.id = meg_coupon_course.coupon_id';
		$sql_2 ="SELECT * FROM (SELECT meg_coupon.id , meg_coupon.coupon_name ,meg_coupon.coupon_code , meg_coupon.discount , meg_coupon_course.course_id FROM meg_coupon LEFT JOIN meg_coupon_course ON meg_coupon.id = meg_coupon_course.coupon_id) AS T WHERE T.coupon_code='$input_coupon_code' AND T.course_id ='$course_id'";
...
//          $data['info']['reg_enquiry'] 	= $this->input->post('reg_enquiry');            
			...
			$data['info']['coupon_discount']= $this->input->post('coupon_discount'); 
...
/*			if($data['info']['coupon_code']){
				$code = $data['info']['coupon_code'];
				$coupon = $this->Meg_model->input_coupon_discount($code);
				...
				/*if($code && !$coupon)
				{
					$data['error']['coupon_code'] = '優惠代碼不正確';
					$valid = false;
				}*/
		...
		//		}
				...
				Array
						(
							[id] => 5
							[coupon_name] => test coupon code2
							[coupon_code] => ABCD
							[discount] => 150
							[allow] => 1
							[creator] => 1
							[status] => 0
							[start_time] => 2018-06-01 15:44:00
							[end_time] => 2018-06-22 15:44:00
							[update_time] => 2018-06-01 15:44:39
							[create_time] => 2018-06-01 15:43:45
						)
				*/ 
//			}
...
//        echo $this->meg_club_discount;
   ...
   */     
		...
		if ($this->session->userdata("USE_COUPON_DISCOUNT")!='0')
        {
						$discount = $this->session->userdata("USE_COUPON_DISCOUNT");
                         $data['price'] -= $discount;
        }
		...
		//	echo 999;
			...
			//	echo 126;
				...
				*/
				 ...
				 if ($this->session->userdata("USE_COUPON_DISCOUNT")!='0')
				   {
						//		$addition_str.=' (已使用 -$'.$this->session->userdata("USE_COUPON_DISCOUNT").' 優惠代碼)';
					 
					 	$used_coupon_input_code =  $this->session->userdata("USED_COUPON_INPUT_CODE");
						$addition_str.=' (已使用優惠代碼'.$used_coupon_input_code.'扣除$'.$this->session->userdata("USE_COUPON_DISCOUNT").')';
					 
					 
					                   $data['price'] -= $this->session->userdata("USE_COUPON_DISCOUNT");
				   }
