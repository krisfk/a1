...
//	}
	...
	public function index($event_code = NULL){
		...
		if ($this->input->post('is_submit')) {
			...
			$data['info']['student_name'] 		= $this->input->post('student_name');
        ...
        //    echo  $data['info']['contact_timeslot'];
	...
	public function pay_return($apply_id){ 
		...
		if ($response->success()){
			...
			if($data['e_pay_email']){
                ...
                <tr>
                <tr>
                  <td class="bold">Join Meg Club：</td>
				  <td>'.$join_megclub_str.'</td>
				</tr>
                <tr>
				  <td class="bold">選擇分店：</td>
				  <td>'.$shop.'</td>
				</tr>
                <tr>
                <tr>
           ...
           $email_html.= '<tr>
              ...
              <tr>
                ...
                <td align="left" valign="middle" class="bold">學員性別：</td>
...
//        print_r($data);
	...
	private function load_header($title, $keywords, $description, $css){
		...
		array_unshift($css, 'css/global.css', 'css/responsive.css','css/magnific-popup.css');
		$data = array(
			'title'			=> $title,
			'keywords'		=> $keywords,
	...
	private function load_footer($js){
		...
		array_unshift($js, 'js/jquery.min.js', 'js/responsive.js','js/jquery.magnific-popup.js','js/global.js');
